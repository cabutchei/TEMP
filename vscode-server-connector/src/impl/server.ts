...
export class FelixRspLauncher {
    ...
    private startServer(
        ...
        // Debuggable version
        this.cpProcess = cp.spawn(java, ['-Xdebug', '-Xrunjdwp:transport=dt_socket,server=y,address=5005,suspend=y', `-Drsp.server.port=${port}`,
            `-Dorg.jboss.tools.rsp.workspace.projects=${workspaceProjects.join(',')}`, '-jar', felix, '-data', storagePath, '-consoleLog'], { cwd: location });
        // Production version
