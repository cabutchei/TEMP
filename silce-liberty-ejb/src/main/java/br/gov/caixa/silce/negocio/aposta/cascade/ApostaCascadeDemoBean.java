...
@Stateless
public class ApostaCascadeDemoBean implements ApostaCascadeDemoLocal {
	...
	@Override
	public Long criarApostaComCascata() {
		Data agora = DataUtil.getTimestampAtual();
		Compra novaCompra = criaNovaCompra(agora);
		return criarApostaComCascata(novaCompra, agora);
	}

	@Override
	public Long criarApostaComCascata(Long compraId) {
		if (compraId == null) {
			throw new IllegalArgumentException("compraId é obrigatório.");
		}

		Compra compra = compraDAO.findById(compraId);
		if (compra == null) {
			throw new IllegalArgumentException("Compra não encontrada para id=" + compraId);
		}
		return criarApostaComCascata(compra, DataUtil.getTimestampAtual());
	}

	private Long criarApostaComCascata(Compra compra, Data agora) {
	...
	private ReservaCotaBolao criaReservaCotaBolao(ApostaMegasena aposta, Compra compra, Data agora) {
		...
		reserva.setId(new ReservaCotaBolaoPK(proximoIdReserva, mesReserva, particaoReserva));
